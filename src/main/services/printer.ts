      ...
      for (const line of lines) {
        ...
        if (line.startsWith('printer ') || line.startsWith('Drucker ')) {
          ...
          if (line.startsWith('printer ')) {
            ...
            name = parts[1];
            status = line.includes('is idle')
              ? 'idle'
              : line.includes('is printing')
              ? 'printing'
              : 'unknown';
            ...
            if (
              ...
              status = line.includes('ist inaktiv')
                ? 'idle'
                : line.includes('druckt')
                ? 'printing'
                : line.includes('ist offline')
                ? 'offline'
                : 'unknown';
      ...
      if (printers.length === 0) {
        ...
        try {
          const { stdout: statusOut } = await execAsync(
            'lpstat -p "Zebra_Technologies_ZTC_GK420d" 2>&1',
          );
...
^XZ`;
  ...
  public async validatePrinter(printerName: string): Promise<boolean> {
    try {
      ...
      // Error('Error validating printer:', error);
      return false;
    }
  }
  ...
  private async updateLabelDate(labelPath: string): Promise<void> {
    try {
      ...
      // Error('Error updating label date:', error);
      throw error;
    }
  }
